...
| Q-014 | Product Readiness / API | P1 | Open | Sustain (`../sustain` `crates/analysis`, pins `stratum-dsp = "1.0"`) consumes the library as low-level primitives (`estimate_bpm_tempogram`, `detect_key`, `chroma::extractor`, `spectral_flux` onsets, `normalization` LUFS), bypassing `analyze_audio` and losing ~7% BPM (~85% vs ~92% per its own comments). Those internal module paths are not a documented/semver-stable public API, so an upstream rename could silently break Sustain. The library lacks a capability-gated analysis surface matching how its primary consumer integrates. | Cross-repo survey 2026-06-03; see PLAN.md "CURRENT NEXT STEP". Sustain also feeds Smart Shuffle from LUFS triplet, onset rate, band ratios, low-band variation, and tonalness built on stratum STFT/onsets. | Two decisions: (1) API shape — A (composable full-quality primitives) / B (bless + document existing internals) / C (capability-flagged `analyze_audio`); (2) consumption model — published crate / git-pinned `open-sustain` fork / full ingestion (Path B). User lean shifted 2026-06-03 toward ingestion + deep clean segmented rework (see PLAN.md "CURRENT NEXT STEP" for footprint + LLM-cost analysis). Note: ingestion contradicts the current AGENTS.md upstream-first mission (upstream has not stalled) — needs explicit sign-off + AGENTS.md update + licensing/provenance work. Constraints: capability-gated performance, no genre profiles (honest `tempo_range` only), whole-range quality. Then write the Sustain-output contract before DSP changes. | Documented, semver-stable surface onto which Sustain's needed outputs (BPM, key, LUFS triplet, onset rate, STFT/chroma for band + tonalness) map; before/after BPM showing the ~7% recovered; no break to the existing public API; decision + rationale recorded. |
