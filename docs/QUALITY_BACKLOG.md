...
| Q-014 | Product Readiness / API | P1 | Open (tracking: sustain#192) | Sustain (`../sustain` `crates/analysis`, pins `stratum-dsp = "1.0"`) consumes the library as low-level primitives (`estimate_bpm_tempogram`, `detect_key`, `chroma::extractor`, `spectral_flux` onsets, `normalization` LUFS), bypassing `analyze_audio` and losing ~7% BPM (~85% vs ~92% per its own comments). Those public-but-low-level module paths are loosely contracted and easy to misuse. Standardizing them around Sustain upstream would impose a larger semver surface primarily for one consumer. | Cross-repo survey 2026-06-03; direction decision recorded 2026-06-04 in PLAN.md and AGENTS.md. Sustain also feeds Smart Shuffle from LUFS triplet, onset rate, band ratios, low-band variation, and tonalness built on stratum STFT/onsets. | Approved split strategy: submit focused, independently valuable fixes upstream; later ingest a minimal call-graph-driven DSP core into Sustain and build its capability-driven interface there. Do not add Sustain-shaped interfaces upstream merely to avoid ingestion, and do not use a long-term git-pinned fork as the destination. Before deeper DSP work: write the Sustain output contract, measure the transitive call graph, complete licensing/provenance work, and establish benchmark baselines. Constraints remain capability-gated performance, no genre profiles, honest tempo-range control, and whole-range quality. | Decision and rationale recorded. Remaining verification: documented Sustain output contract; measured ingest call graph; retained license/provenance; benchmark baselines; before/after BPM showing whether the ~7% gap is recovered. |

## Progress Log

### 2026-06-06

- Amended `Q-014`: dropped the upstream `symphonia` hygiene PR and moved to an
  ingest-only direction. Canonical tracking now lives in `open-sustain/sustain`
  issue #192; the ingest source of record is
  `open-sustain/stratum-dsp@wip/dsp-rework` (it carries our fork fixes to
  `detect_key`/`chroma` plus input hardening and config validation that the
  published `stratum-dsp 1.0` crate lacks). This repo's planning docs are now
  decision history. No code change.

### 2026-06-04

- Resolved `Q-014`: approved a dual-track direction. Focused changes with
  independent general value continue upstream; Sustain will later ingest the
  minimal transitive DSP core needed for its capability-driven architecture and
  deeper rework. A Sustain-shaped standardized upstream API and a long-term
  git-pinned fork were rejected as destination architectures. Updated
  `AGENTS.md` and `PLAN.md`; output-contract, call-graph, provenance, and
  benchmark work remains open. No code change.

### 2026-06-03

- Updated `Q-014`: added the consumption-model axis (published / git-pinned fork
  / ingest) and two analyses to the Codex handoff — (1) a tailored ingest is
  ~half the ~20k-LOC crate or less (`beat_tracking`, confidence, orchestration,
  `waveform`, `ml` drop immediately; ~5k LOC); (2) LLM-era free dev labor lowers
  all options equally and shifts the decision onto validation/divergence/
  ownership, which ingestion concentrates on Sustain. User now leans ingestion +
  deep rework; flagged the AGENTS.md upstream-first mission tension for Codex to
  resolve with the user. No code change.
- Recorded `Q-014`: cross-repo survey found Sustain consumes stratum-dsp as
  low-level primitives (not `analyze_audio`), losing ~7% BPM and pinning
  undocumented internal module paths. Boundary architecture decision (A/B/C) is
  handed to Codex; see PLAN.md "CURRENT NEXT STEP". No code change.
